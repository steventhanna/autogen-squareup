...
/// CustomerCreatedEvent : Published when a [customer](entity:Customer) is created. Subscribe to this event to track customer profiles affected by a merge operation. For more information, see [Use Customer Webhooks](https://developer.squareup.com/docs/customers-api/use-the-api/customer-webhooks).  The `customer` object in the event notification does not include the `segment_ids` field.
...
pub struct CustomerCreatedEvent {
    ...
    pub data: Option<Box<models::CustomerCreatedEventData>>,
}
...
impl CustomerCreatedEvent {
    ...
    pub fn new() -> CustomerCreatedEvent {
