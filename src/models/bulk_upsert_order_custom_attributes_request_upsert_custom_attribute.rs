/*
 * Square
 *
 * Use Square APIs to manage and run business including payment, customer, product, inventory, and employee management.
 *
 * The version of the OpenAPI document: 2.0
 * Contact: developers@squareup.com
 * Generated by: https://openapi-generator.tech
 */

use crate::models;
use serde::{Deserialize, Serialize};

/// BulkUpsertOrderCustomAttributesRequestUpsertCustomAttribute : Represents one upsert within the bulk operation.
#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize)]
pub struct BulkUpsertOrderCustomAttributesRequestUpsertCustomAttribute {
    #[serde(rename = "custom_attribute")]
    pub custom_attribute: Box<models::CustomAttribute>,
    /// A unique identifier for this request, used to ensure idempotency.  For more information, see [Idempotency](https://developer.squareup.com/docs/build-basics/common-api-patterns/idempotency).
    #[serde(rename = "idempotency_key", default, with = "::serde_with::rust::double_option", skip_serializing_if = "Option::is_none")]
    pub idempotency_key: Option<Option<String>>,
    /// The ID of the target [order](entity:Order).
    #[serde(rename = "order_id")]
    pub order_id: String,
}

impl BulkUpsertOrderCustomAttributesRequestUpsertCustomAttribute {
    /// Represents one upsert within the bulk operation.
    pub fn new(custom_attribute: models::CustomAttribute, order_id: String) -> BulkUpsertOrderCustomAttributesRequestUpsertCustomAttribute {
        BulkUpsertOrderCustomAttributesRequestUpsertCustomAttribute {
            custom_attribute: Box::new(custom_attribute),
            idempotency_key: None,
            order_id,
        }
    }
}

