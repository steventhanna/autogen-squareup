...
/// CustomerCustomAttributeDefinitionVisibleDeletedEvent : Published when a customer [custom attribute definition](entity:CustomAttributeDefinition) that is visible to the subscribing application is deleted. A custom attribute definition can only be deleted by the application that created it. A notification is sent when your application deletes a custom attribute definition or when another application deletes a custom attribute definition whose `visibility` is `VISIBILITY_READ_ONLY` or `VISIBILITY_READ_WRITE_VALUES`.
#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize)]
pub struct CustomerCustomAttributeDefinitionVisibleDeletedEvent {
...
impl CustomerCustomAttributeDefinitionVisibleDeletedEvent {
    ...
    pub fn new() -> CustomerCustomAttributeDefinitionVisibleDeletedEvent {
